...
SkillLoop does not treat one transcript as sufficient evidence. By default, a cluster becomes eligible after the same sanitized fingerprint appears in three distinct sessions.
...
The text view prints the exact committed diff between `BaseCommit` and `CandidateCommit`; the JSON view also exposes those revisions, the worktree path, and whether the candidate requires human approval.
...
The continuous daemon runs a monitoring pass after every drain. You can also run monitoring independently, once or continuously:
...
Monitoring re-evaluates each active promotion. A completed regression rolls back automatically; evaluator or restoration errors fail safe, leave the release active, and are reported as failures. You can always roll back manually:
...
- **Codex:** [user hooks](https://developers.openai.com/codex/hooks) in `~/.codex/hooks.json`; JSONL transcripts are normalized from `response_item` messages, function/custom tool calls and outputs, and relevant `event_msg` records.
- **Claude Code:** [user hooks](https://code.claude.com/docs/en/hooks) in `~/.claude/settings.json`; JSONL transcripts are normalized from text, `tool_use`, and `tool_result` content blocks.
