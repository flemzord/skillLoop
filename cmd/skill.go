	...
	command := &cobra.Command{
		...
		RunE: func(command *cobra.Command, _ []string) error {
			...
			for _, skill := range skills {
				if _, err := fmt.Fprintf(command.OutOrStdout(), "%s\t%s\t%s\n", terminalSafe(skill.ID), terminalSafe(skill.Name), terminalSafe(filepath.Join(skill.RepositoryPath, skill.InstructionPath))); err != nil {
					return err
				}
			}
			return nil
...
func validateSkillRepository(ctx context.Context, repository, instruction string) (string, string, error) {
	...
	rootOutput, err := runSkillGit(ctx, absolute, "rev-parse", "--show-toplevel")
	if err != nil {
		return "", "", fmt.Errorf("repository is not a Git worktree: %w", err)
	}
	root := filepath.Clean(strings.TrimSpace(string(rootOutput)))
	if root != absolute {
		return "", "", fmt.Errorf("register the repository root %s instead of %s", terminalSafe(root), terminalSafe(absolute))
	}
	...
	if _, err := runSkillGit(ctx, absolute, "ls-files", "--error-unmatch", "--", relative); err != nil {
