...
func TestReadDocumentRejectsUnsafeFilesAndOversizedInput(t *testing.T) {
	t.Run("symlink", func(t *testing.T) {
		...
		if _, err := readDocument(path); err == nil || !strings.Contains(err.Error(), "not a regular file") {
	...
	t.Run("FIFO", func(t *testing.T) {
		...
		case <-time.After(time.Second):
			t.Fatal("reading a FIFO hooks config blocked")
		}
	})
	...
	t.Run("device", func(t *testing.T) {
		...
		if _, err := readDocument("/dev/null"); err == nil || !strings.Contains(err.Error(), "not a regular file") {
	...
	t.Run("oversized", func(t *testing.T) {
		path := filepath.Join(t.TempDir(), "hooks.json")
		contents := bytes.Repeat([]byte(" "), int(maxHooksConfigFileBytes)+1)
		if err := os.WriteFile(path, contents, 0o600); err != nil {
			t.Fatal(err)
		}
		if _, err := readDocument(path); err == nil || !strings.Contains(err.Error(), "exceeds") {
