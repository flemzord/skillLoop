...
const maxHooksConfigFileBytes int64 = 1 << 20
...
func readDocument(path string) (map[string]json.RawMessage, error) {
	contents, err := readDocumentFile(path)
...
func readDocumentFile(path string) ([]byte, error) {
	...
	if !expected.Mode().IsRegular() {
		return nil, fmt.Errorf("hooks config is not a regular file: %s", expected.Mode().Type())
	}
	if expected.Size() < 0 || expected.Size() > maxHooksConfigFileBytes {
		return nil, fmt.Errorf("hooks config exceeds %d bytes", maxHooksConfigFileBytes)
	}

	fd, err := unix.Open(path, unix.O_RDONLY|unix.O_CLOEXEC|unix.O_NOFOLLOW|unix.O_NONBLOCK, 0)
	if err != nil {
		return nil, fmt.Errorf("open hooks config safely: %w", err)
	}
	file := os.NewFile(uintptr(fd), path)
	if file == nil {
		_ = unix.Close(fd)
		return nil, errors.New("open hooks config safely: invalid file descriptor")
	}
	defer func() { _ = file.Close() }()

	opened, err := file.Stat()
	if err != nil {
		return nil, fmt.Errorf("inspect opened hooks config: %w", err)
	}
	if !opened.Mode().IsRegular() {
		return nil, fmt.Errorf("opened hooks config is not a regular file: %s", opened.Mode().Type())
	}
	if !os.SameFile(expected, opened) {
		return nil, errors.New("hooks config changed while it was being opened")
	}
	if opened.Size() < 0 || opened.Size() > maxHooksConfigFileBytes {
		return nil, fmt.Errorf("hooks config exceeds %d bytes", maxHooksConfigFileBytes)
	}

	contents, err := io.ReadAll(io.LimitReader(file, maxHooksConfigFileBytes+1))
	if err != nil {
		return nil, fmt.Errorf("read hooks config safely: %w", err)
	}
	if int64(len(contents)) > maxHooksConfigFileBytes {
		return nil, fmt.Errorf("hooks config exceeds %d bytes", maxHooksConfigFileBytes)
	}
	return contents, nil
}
