...
func (service Service) Prepare(ctx context.Context, skill domain.Skill, cluster domain.Cluster) (Candidate, error) {
	...
	worktreeParent, err := openStateDirectory(service.StateDir, "worktrees", safeName(skill.ID))
	if err != nil {
		return Candidate{}, err
	}
	defer func() { _ = worktreeParent.Close() }()
	worktreeName := safeName(cluster.ID) + "-" + suffix
	worktree := filepath.Join(worktreeParent.path, worktreeName)
	branch := "skillloop/" + safeName(skill.ID) + "/" + safeName(cluster.ID) + "-" + suffix
	worktreeDirectory, _, err := addWorktreeAt(ctx, repository, worktreeParent, worktreeName, worktree, base, branch)
	if err != nil {
		return Candidate{}, err
	}
	defer func() { _ = worktreeDirectory.Close() }()
	keepWorktree := false
	defer func() {
		if keepWorktree {
			return
		}
		_, _ = gitAt(context.Background(), worktreeDirectory, "worktree", "remove", "--force", ".")
		_, _ = git(context.Background(), repository, "branch", "-D", branch)
	}()

	worktreeState := &stateDirectory{path: worktree, file: worktreeDirectory}
	instruction, err := openStateFile(worktreeState, relative, unix.O_RDWR)
	if err != nil {
		return Candidate{}, fmt.Errorf("open candidate SKILL.md: %w", err)
	}
	defer func() { _ = instruction.Close() }()
	currentContents, info, err := readOpenFileLimit(instruction, maxSkillFileBytes)
	if err != nil {
	...
	if _, err := instruction.Seek(0, 0); err != nil {
		return Candidate{}, fmt.Errorf("rewind candidate SKILL.md: %w", err)
	}
	if err := instruction.Truncate(0); err != nil {
		return Candidate{}, fmt.Errorf("truncate candidate SKILL.md: %w", err)
	}
	if _, err := instruction.Write(updated); err != nil {
		return Candidate{}, fmt.Errorf("write candidate SKILL.md: %w", err)
	}
	if err := instruction.Chmod(info.Mode().Perm()); err != nil {
		return Candidate{}, fmt.Errorf("preserve candidate SKILL.md mode: %w", err)
	}

	changed, err := gitWithoutFiltersAt(ctx, repository, worktreeDirectory, "diff", "--no-ext-diff", "--no-textconv", "--name-only")
	if err != nil {
		return Candidate{}, err
	}
	if changed != relative {
	...
	diff, err := gitWithoutFiltersAt(ctx, repository, worktreeDirectory, "diff", "--no-ext-diff", "--no-textconv", "--", relative)
	if err != nil {
		return Candidate{}, err
	}
	...
	if _, err := gitWithoutFiltersAt(ctx, repository, worktreeDirectory, "add", "--", relative); err != nil {
		return Candidate{}, err
	}
	...
	if _, err := gitWithoutFiltersAt(ctx, repository, worktreeDirectory,
	...
	candidateCommit, err := gitWithoutFiltersAt(ctx, repository, worktreeDirectory, "rev-parse", "HEAD^{commit}")
	if err != nil {
		return Candidate{}, err
	}
	...
	committedDiff, err := git(ctx, repository, "diff", "--no-ext-diff", "--no-textconv", base, candidateCommit, "--", relative)
	if err != nil {
		return Candidate{}, err
	}
	...
	if err := verifyStateChildIdentity(worktreeParent, worktreeName, worktreeDirectory); err != nil {
		return Candidate{}, fmt.Errorf("verify candidate worktree namespace: %w", err)
	}
...
// considered for autopilot. Its lesson must be one exact canonical command
// emitted by the analyzer, and textual risk markers still make it fail closed.
func classifyCandidate(kind domain.CardKind, summary, lesson string) (bool, error) {
	...
	case domain.CardValidation:
		text := summary + "\n" + lesson
		return !autopilotValidationLesson(lesson) || sensitiveChange.MatchString(text) || promptInjection.MatchString(text), nil
	default:
		return true, fmt.Errorf("unsupported learning card kind %q: %w", kind, ErrUnsafeChange)
	}
}

func autopilotValidationLesson(lesson string) bool {
	switch lesson {
	case "go test ./...", "golangci-lint run", "nix flake check", "pytest",
		"cargo test", "npm test", "pnpm test", "just test":
		return true
	default:
		return false
	}
