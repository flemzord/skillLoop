...
type Job struct {
	...
	Status         JobStatus
	Attempts       int
	// FencingToken identifies the exact lease generation held by a worker.
	// Workers must present it when completing or retrying a job so an expired
	// worker cannot overwrite the result of a newer lease owner.
	FencingToken int
